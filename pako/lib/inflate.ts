import { FlushMode, Z_FINISH, Z_NO_FLUSH } from './zlib/flush-mode';
...
import { } from './zlib/inflate';
import InflateState from './zlib/inflate-state';
import { ReturnCode, Z_DATA_ERROR, Z_MEM_ERROR, Z_NEED_DICT, Z_OK, Z_STREAM_END, Z_STREAM_ERROR } from './zlib/return-code';
...
export class Inflate {
   ...
   * ```
   */
  readonly push = (data: Uint8Array, flush_mode: FlushMode | boolean = Z_NO_FLUSH) => {
   ...
   */
  readonly onData = (chunk: Uint8Array) => {
   ...
   */
  readonly onEnd = (status: ReturnCode) => {
    // On success - join
    if (status === Z_OK) {
      /*
      if (this.options.to === 'string') {
 ...
 * ```
 */
export const inflate = (input: Uint8Array, options: any) => {
 ...
 */
export const inflateRaw = (input: Uint8Array, options: any) => {
  options = options || {};
  options.raw = true;
